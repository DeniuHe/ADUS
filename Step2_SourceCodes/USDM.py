...
class USME():
    ...
    def alm(self, K, b, k=1):
        ...
        for it in range(100):
            ...
            if (obj_prev is not None) and (abs(obj_prev - obj) < 1e-6):
                break
        return f
    ...
    def select(self):
        while self.budgetLeft > 0:
            ...
            for i, idx in enumerate(self.unlabeled):
                ...
                b.append(entrop)
            K = self.K[np.ix_(self.unlabeled, self.unlabeled)]
            # print("K::",K.shape,"len(unlabeled)::",len(self.unlabeled))

            f = self.alm(K=K,b=b)
            ord_idx = np.argsort(f)[-1]
            tar_idx = self.unlabeled[ord_idx]
...
if __name__ == '__main__':
    ...
    for name in names_list:
        ...
        save_path = Path(r"E:\FFFFF\SelectedResult\USDM")
